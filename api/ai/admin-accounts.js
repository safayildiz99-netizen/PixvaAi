...
export default async function handler(req,res){
  try{
    ...
    const projectMap=grouped(projects,'owner_id'),knowledgeMap=grouped(knowledge),agentMap=grouped(agents),usageMap=grouped(usage),approvalMap=grouped(approvals),sessionMap=grouped(sessions),chatMap=grouped(chats);const factorMap=new Map(factors.map(x=>[x.user_id,Boolean(x.enabled)]));
