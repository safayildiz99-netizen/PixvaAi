...
export async function getPixvaBrainContext(user){
  ...
  const company={
    ...
    openingHours:first(profile.opening_hours,brand.opening_hours,mem['öffnungszeiten']),
    logoDataUrl:first(profile.logo_data_url,brand.logo_data_url),
    logoUrl:signedLogo,
    logoPath,
    primaryColor:first(profile.primary_color,brand.primary_color,mem['primärfarbe'],defaults.primary),
    secondaryColor:first(profile.secondary_color,brand.secondary_color,mem['sekundärfarbe'],defaults.secondary),
    fontFamily:first(profile.font_family,brand.font_family,'Inter'),
    designStyle:first(profile.design_style,brand.design_style,mem['designstil'],defaults.style),
