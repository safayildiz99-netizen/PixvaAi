...
import { Cpu, Plus, Save, Shield, UserRoundCog } from 'lucide-react';
import { api } from '../api.js';

export default function Admin({ user, onUserChanged }) {
  const [users,setUsers]=useState([]); const [status,setStatus]=useState('');
  const [newUser,setNewUser]=useState({username:'',password:'',role:'user'});
  const [passwords,setPasswords]=useState({currentPassword:'',newPassword:''});
  async function load(){try{const u=await api('/api/users');setUsers(u.users)}catch(e){setStatus(e.message)}}
  useEffect(()=>{load()},[]);
  ...
  async function changePassword(e){e.preventDefault();try{await api('/api/auth/change-password',{method:'POST',body:JSON.stringify(passwords)});setPasswords({currentPassword:'',newPassword:''});setStatus('Passwort geändert. Bitte nach dem Abmelden das neue Passwort verwenden.');onUserChanged?.({...user,mustChangePassword:false})}catch(e){setStatus(e.message)}}
  return <section className="admin-page"><div className="page-heading"><div><h2><Shield size={22}/> Admin & Einstellungen</h2><p>Konten ohne sichtbare E-Mail und lokale KI verwalten.</p></div></div>{status&&<div className="status-line">{status}</div>}<div className="admin-grid">
    <article className="admin-card"><h3><Cpu size={19}/> Kostenlose lokale KI</h3><p>Status: <b>Keine API, kein Guthaben, keine KI-Credits</b></p><div className="info-box">Der Chat läuft direkt auf dem Gerät des Nutzers. Es gibt kein Nachrichtenlimit und keine Zahlungsaufforderung. Beim ersten Start wird ein Modell heruntergeladen. Die Anmeldung nutzt weiterhin einen sicheren Sitzungsschlüssel; das ist kein Bezahl- oder Usage-System.</div><small>Für beste Leistung Chrome oder Edge auf einem Computer verwenden.</small></article>
