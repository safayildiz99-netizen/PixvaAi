...
export default function Admin({ user, onUserChanged }) {
  ...
  return <section className="admin-page"><div className="page-heading"><div><h2><Shield size={22}/> Admin & Einstellungen</h2><p>Konten ohne E-Mail und KI-Verbindung verwalten.</p></div></div>{status&&<div className="status-line">{status}</div>}<div className="admin-grid">
    <article className="admin-card"><h3><KeyRound size={19}/> KI-Verbindung</h3><p>Status: <b>{settings?.hasPollinationsKey?'bereit':'noch nicht eingerichtet'}</b></p><div className="info-box">Der KI-Schlüssel wird sicher bei Vercel gespeichert. Öffne dort <b>Settings → Environment Variables</b> und füge <b>POLLINATIONS_KEY</b> hinzu. Für Videos sollte der Schlüssel mit <b>pk_</b> beginnen.</div><small>Geheime Schlüssel niemals in GitHub hochladen.</small></article>
