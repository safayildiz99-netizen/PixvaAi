...
export default function Admin({ user, onUserChanged }) {
  ...
  return <section className="admin-page"><div className="page-heading"><div><h2><Shield size={22}/> Admin & Einstellungen</h2><p>Konten ohne sichtbare E-Mail und Gemini-Verbindung verwalten.</p></div></div>{status&&<div className="status-line">{status}</div>}<div className="admin-grid">
    <article className="admin-card"><h3><Cpu size={19}/> Gemini über Vercel</h3><p>Status: <b>Keine lokale GPU erforderlich</b></p><div className="info-box">Der Chat läuft über eine geschützte Vercel-Funktion mit Gemini. Er funktioniert browserunabhängig in Opera, Chrome, Safari und Edge. Das kostenlose Google-Kontingent kann zeitweise ausgelastet sein; Yildiz AI wechselt dann automatisch zwischen mehreren aktuellen Modellen.</div><small>Gastzugang ist aktiv. Anmeldung wird nur zum Speichern und für Adminfunktionen benötigt.</small></article>
