...
export default function Admin({ user, onUserChanged }) {
  ...
  return <section className="admin-page"><div className="page-heading"><div><h2><Shield size={22}/> Admin & Einstellungen</h2><p>Konten ohne E-Mail, kostenlose KI-Fallbacks und Einstellungen verwalten.</p></div></div>{status&&<div className="status-line">{status}</div>}<div className="admin-grid">
    <article className="admin-card"><h3><KeyRound size={19}/> KI-Verbindung</h3><p>Status: <b>{settings?.hasPollinationsKey?'API-Key verbunden':'kostenloser Fallback aktiv'}</b></p><div className="info-box">Text und Bilder versuchen ohne Guthaben automatisch einen kostenlosen öffentlichen Fallback. Dieser kann langsam oder zeitweise nicht verfügbar sein. Echte KI-Videos benötigen weiterhin Guthaben; der Video-Editor selbst bleibt kostenlos.</div><small>Geheime Schlüssel niemals in GitHub hochladen.</small></article>
