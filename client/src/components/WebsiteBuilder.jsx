...
export default function WebsiteBuilder({ project, onSaved }) {
  ...
  async function aiText(){ setAiLoading(true); setStatus('Texte werden erstellt …'); try{ const r=await api('/api/ai/chat',{method:'POST',body:JSON.stringify({prompt:`Schreibe für die Werbetechnik-Firma ${data.company} einen kurzen starken Slogan und einen Einleitungstext. Antworte in zwei Zeilen: SLOGAN: ... und TEXT: ...`})}); const sm=r.answer.match(/SLOGAN:\s*(.*)/i); const tx=r.answer.match(/TEXT:\s*(.*)/i); setData((o)=>({...o,slogan:sm?.[1]||o.slogan,intro:tx?.[1]||r.answer})); setStatus('Texte übernommen.'); }catch(e){setStatus(e.message)} finally{setAiLoading(false)} }
